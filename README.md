...
- Automated validation with **365 passing tests across 11 test files** (`npm run test:run` on 2026-07-05), plus an opt-in real-provider E2E test (skipped without `CODEGUARD_E2E=1` + API key) and a CI smoke job exercising the composite Action against the fixtures
...
| M6 More languages | Done | Go: 9 rules; Java: 11 rules (adds `CG-010`, `CG-031`, `CG-041` over the Go set); PHP: 11 rules (adds `CG-021`/`CG-031`/`CG-040`/`CG-041`/`CG-050` over the original MVP set) |
...
| Python | `.py` | 12 of 13 built-in rules (all but `CG-011` DOM-based XSS, which needs a browser DOM) |
| Go | `.go` | `CG-001` SQL injection, `CG-002` command injection, `CG-020` hardcoded credentials, `CG-021` weak crypto (`crypto/md5`/`sha1`/`des`/`rc4`), `CG-030` path traversal, `CG-031` arbitrary file access, `CG-040` sensitive data exposure (`log`/`logrus`/`zap`), `CG-050` security misconfiguration (`InsecureSkipVerify`), `CG-060` SSRF (9 rules) |
| Java | `.java` | `CG-001` SQL injection (incl. `String.format`), `CG-002` command injection (`Runtime.exec`, `ProcessBuilder`), `CG-010` XSS (unescaped `response.getWriter().write(...)`), `CG-020` hardcoded credentials, `CG-021` weak crypto (`MessageDigest`/`Cipher`), `CG-030` path traversal (`File`/`Files`/`Paths`), `CG-031` arbitrary file access, `CG-040` sensitive data exposure, `CG-041` insecure deserialization (`ObjectInputStream#readObject`), `CG-050` security misconfiguration (Spring CSRF/CORS, cookie flags), `CG-060` SSRF (`URL`, RestTemplate) (11 rules) |
| PHP | `.php` | `CG-001` SQL injection (`mysqli_query`, PDO/mysqli `->query`, `Class::query` facades), `CG-002` command injection (`exec`, `shell_exec`, `passthru`, `proc_open`), `CG-003` eval/code injection, `CG-020` hardcoded credentials, `CG-021` weak crypto (`md5`/`sha1`/`hash()`), `CG-030` path traversal (`file_get_contents`, `fopen`, etc.), `CG-031` arbitrary file access, `CG-040` sensitive data exposure (`error_log`, `Log::`, `$logger->`), `CG-041` insecure deserialization (`unserialize`), `CG-050` security misconfiguration (curl TLS verification, `display_errors`), `CG-060` SSRF (`curl_init`) (11 rules) |
...
| Auth / Crypto | `CG-020`, `CG-021` | Hardcoded credentials (also Go, Java, and PHP), weak cryptography (also Go, Java, and PHP) |
...
- Go covers 9 rules (`CG-001`/`CG-002`/`CG-020`/`CG-021`/`CG-030`/`CG-031`/`CG-040`/`CG-050`/`CG-060`); Java covers those plus `CG-010`/`CG-041` (11 total); PHP covers those plus `CG-003`/`CG-041` (11 total). `CG-011` (DOM-based XSS) is JS/TS-only since it needs a browser DOM
