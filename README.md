...
- Automated validation with **380 passing tests across 11 test files** (`npm run test:run` on 2026-07-05), plus an opt-in real-provider E2E test (skipped without `CODEGUARD_E2E=1` + API key) and a CI smoke job exercising the composite Action against the fixtures
...
| M6 More languages | Done | Go: 11 rules; Java: 13 rules (adds `CG-010`, `CG-031`, `CG-041` over the Go set); PHP: 13 rules (adds `CG-021`/`CG-031`/`CG-040`/`CG-041`/`CG-050` over the original MVP set) |
...
|----------|------------|---------------|
| JavaScript | `.js`, `.jsx`, `.mjs`, `.cjs` | all 15 built-in rules |
| TypeScript | `.ts`, `.tsx` | all 15 built-in rules |
| Python | `.py` | 14 of 15 built-in rules (all but `CG-011` DOM-based XSS, which needs a browser DOM) |
...
- Go covers 11 rules (`CG-001`/`CG-002`/`CG-020`/`CG-021`/`CG-022`/`CG-023`/`CG-030`/`CG-031`/`CG-040`/`CG-050`/`CG-060`); Java covers those plus `CG-010`/`CG-041` (13 total); PHP covers those plus `CG-003`/`CG-041` (13 total). `CG-011` (DOM-based XSS) is JS/TS-only since it needs a browser DOM
