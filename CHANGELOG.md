...
- **New rule: `CG-024` NoSQL Injection (CWE-943)**, covering JS/TS, Python, and PHP — a third new rule category, distinct from `CG-001` SQL injection since it's about object *structure* (query operators), not string concatenation. Flags a MongoDB CRUD method (`find`/`findOne`/`updateOne`/`deleteOne`/etc.; pymongo's snake_case equivalents on the Python side — `find_one`/`update_one`/etc., since only `find`/`aggregate` are spelled the same as the JS/PHP camelCase driver APIs) called with the *entire* request object as the filter/update document (`users.find(req.body)`), which lets an attacker submit query operators (`$ne`, `$gt`, ...) instead of a plain value and bypass checks like a password comparison entirely. A specific field access (`req.body.username`) is an ordinary string value and isn't flagged — only the whole object being passed directly is the signal. Also flags a `$where` clause built with string concatenation/template literals, which MongoDB evaluates as arbitrary JavaScript (the NoSQL analogue of `CG-003`). Not ported to Go/Java: their MongoDB driver APIs are more strongly typed and don't have a matching "pass a raw object directly" call shape. 9 new tests plus fixture coverage across all 3 languages. Built-in rule count: 15 → 16.
