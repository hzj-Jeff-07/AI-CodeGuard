...
- 规则来源：**17 条 TypeScript 内置规则 + 可选 YAML custom rules**
...
- 支持语言：JavaScript / TypeScript（全部 17 条规则）/ Python（16 条，仅缺 `CG-011` DOM XSS）/ Go（12 条）/ Java（14 条）/ PHP（15 条，详见第 9 节）
...
- `preset = owasp-top-10`：当前返回全部 17 条 built-in rules
- `preset = all`：当前也返回全部 17 条 built-in rules
...
1. **Go / Java / PHP 覆盖范围**
   - Go 支持 `CG-001` / `CG-002` / `CG-020` / `CG-021` / `CG-022` / `CG-023` / `CG-025` / `CG-030` / `CG-031` / `CG-040` / `CG-050` / `CG-060` 共 12 条；Java 在此基础上再加 `CG-010` / `CG-041` 共 14 条；PHP 在 Go 的基础上再加 `CG-003` / `CG-024` / `CG-041` 共 15 条。`CG-011`（DOM-based XSS）仍是 JS/TS 独有，因为它需要浏览器 DOM 环境；`CG-024`（NoSQL 注入）只覆盖 JS/TS/Python/PHP，因为它针对的是 MongoDB 驱动特定的调用形态，Go/Java 的驱动 API 类型更严格，没有清晰的对等模式。
