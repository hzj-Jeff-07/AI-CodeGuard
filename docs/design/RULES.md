...
| `CG-003` | Code Injection (eval) | critical | JS / TS / Python / PHP | `eval` / `Function` / `setTimeout` / `setInterval` 等危险调用；PHP 的 `eval()` 在 tree-sitter-php 语法中就是普通函数调用节点，复用同一份 `EVAL_FUNCTIONS` 列表即可命中；Python 侧额外覆盖内建 `exec(...)`（限定裸调用、无接收者——JS 的 `child_process.exec`、PHP 的 `exec()` 是命令执行，归 CG-002，不在此命中） |
