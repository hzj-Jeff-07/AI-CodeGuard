...
- 支持语言：JavaScript / TypeScript（全部 13 条规则）/ Python（12 条，仅缺 `CG-011` DOM XSS）/ Go（9 条）/ Java（11 条）/ PHP（11 条，详见第 9 节）
...
1. **Go / Java / PHP 覆盖范围**
   - Go 支持 `CG-001` / `CG-002` / `CG-020` / `CG-021` / `CG-030` / `CG-031` / `CG-040` / `CG-050` / `CG-060` 共 9 条；Java 在此基础上再加 `CG-010` / `CG-041` 共 11 条；PHP 在 Go 的基础上再加 `CG-003` / `CG-041` 共 11 条。`CG-011`（DOM-based XSS）仍是 JS/TS 独有，因为它需要浏览器 DOM 环境。
